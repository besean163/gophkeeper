	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func() {
				...
				changeDetector.EXPECT().GetNoteChanges(gomock.Any(), gomock.Any()).Return(
					changes.NoteChanges{
						Created: []models.Note{{UUID: "uuid"}},
						Updated: []models.Note{},
						Deleted: []models.Note{},
					},
				).Times(1)
