...
func TestDeleteCard(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func() {
				...
				httpClient.EXPECT().Delete("/api/card", entities.CardDeleteInput{}, map[string]string{
		...
		},
		{
			name: "fail",
			mockSetup: func() {
				httpClient.EXPECT().Delete(gomock.Any(), gomock.Any(), gomock.Any()).Return(response, errors.New("test error")).Times(1)
			},
		...
		},
		{
			...
			mockSetup: func() {
				...
				httpClient.EXPECT().Delete(gomock.Any(), gomock.Any(), gomock.Any()).Return(response, nil).Times(1)
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := client.DeleteCard(entities.CardDeleteInput{})
