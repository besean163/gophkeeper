...
func TestNoteDeleteRoute(t *testing.T) {
	...
	handler := route.NoteDeleteRoute(debs)
	...
	tests := []struct {
		...
		},
		{
			...
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().DeleteNote(gomock.Any(), gomock.Any()).Return(errors.New("test")).Times(1)
			},
		},
	}
