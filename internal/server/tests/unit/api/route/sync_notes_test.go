...
func TestNotesSyncRoute(t *testing.T) {
	...
	handler := route.NotesSyncRoute(debs)
	...
	tests := []struct {
		...
		{
			name:        "success",
			requestBody: `{"notes":[{"uuid":"uuid_1","name":"name_1","content":"text_1","created_at":1,"updated_at":1,"deleted_at":1,"synced_at":1}]}`,
			exceptCode:  http.StatusOK,
			user:        &user,
			mockSetup: func() {
				bucketService.EXPECT().SyncNotes(gomock.Any(), gomock.Any(), []models.ExternalNote{
					{
						UUID:      "uuid_1",
						Name:      "name_1",
						Content:   "text_1",
		...
		},
		{
			...
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().SyncNotes(gomock.Any(), gomock.Any(), gomock.Any()).Return(errors.New("test_error")).Times(1)
			},
		},
	}
