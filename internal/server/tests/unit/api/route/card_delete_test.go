...
func TestCardDeleteRoute(t *testing.T) {
	...
	handler := route.CardDeleteRoute(debs)
	...
	tests := []struct {
	...
	}{
		{
			...
			user:        &user,
			mockSetup: func() {
				bucketService.EXPECT().DeleteCard(user, "00000000-0000-0000-0000-000000000000").Return(nil).Times(1)
			},
		},
		{
			name:         "empty_uuid",
		...
		},
		{
			...
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().DeleteCard(user, "00000000-0000-0000-0000-000000000000").Return(apierrors.ErrorNotFoundByUUID).Times(1)
			},
		},
		{
			name:         "invalid_json",
		...
		},
		{
			...
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().DeleteCard(gomock.Any(), gomock.Any()).Return(errors.New("test")).Times(1)
			},
		},
	}
