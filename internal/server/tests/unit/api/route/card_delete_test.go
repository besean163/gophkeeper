...
func TestCardDeleteRoute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			user:        &user,
			mockSetup: func() {
				bucketService.EXPECT().DeleteCard(user, "test_uuid").Return(nil).Times(1)
			},
		},
		{
			name:         "empty_uuid",
		...
		},
		{
			...
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().DeleteCard(user, "not_found_uuid").Return(apierrors.ErrorNotFoundByUUID).Times(1)
			},
		},
		{
			name:         "invalid_json",
