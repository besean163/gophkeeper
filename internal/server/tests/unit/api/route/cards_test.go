...
func TestCardsRoute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			responseBody: fmt.Sprintf(`{"cards":[{"uuid":"uuid_1","name":"name_1","number":1111,"exp":"00|00","cvv":111,"created_at":%d,"updated_at":%d}]}`, now.Unix(), now.Unix()),
			exceptCode:   http.StatusOK,
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().GetCards(user).Return([]*models.Card{
					{
						UUID:      "uuid_1",
						Name:      "name_1",
						Number:    1111,
		...
		},
		{
			...
			user:         &user,
			mockSetup: func() {
				bucketService.EXPECT().GetCards(user).Return(nil, errors.New("test")).Times(1)
			},
		},
	}
