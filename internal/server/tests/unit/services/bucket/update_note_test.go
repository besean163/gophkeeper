...
func TestUpdateNote(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "success",
			item: &models.Note{UserID: 1, UUID: "uuid_1", Name: "new_name_1"},
			mockSetup: func() {
				repository.EXPECT().GetNote("uuid_1").Return(&models.Note{UserID: 1, UUID: "uuid_1", Name: "name_1"}, nil).Times(1)
				timecontroller.EXPECT().Now().Return(int64(1)).Times(1)
				repository.EXPECT().SaveNote(&models.Note{UserID: 1, UUID: "uuid_1", Name: "new_name_1", UpdatedAt: 1}).Return(nil).Times(1)
			},
		},
		{
			name: "not_found",
			item: &models.Note{UserID: 1, UUID: "uuid_1", Name: "new_name_1"},
			mockSetup: func() {
				repository.EXPECT().GetNote("uuid_1").Return(nil, nil).Times(1)
			},
		...
		},
		{
			...
			item: &models.Note{UserID: 1, UUID: "uuid_1", Name: "new_name_1"},
			mockSetup: func() {
				repository.EXPECT().GetNote("uuid_1").Return(nil, errors.New("test_error")).Times(1)
			},
